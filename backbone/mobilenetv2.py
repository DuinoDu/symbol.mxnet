...
def Conv1x1(channels, linear=False, **kwargs):
    out = nn.HybridSequential(**kwargs)
    with out.name_scope():
        out.add(
            nn.Conv2D(channels, 1, strides=1, padding=0, use_bias=False),
            nn.BatchNorm(scale=True)
        )
        if not linear:
    ...
    def __init__(self, inp, oup, t, strides, same_shape=True, **kwargs):
        ...
        with self.name_scope():
            ...
            self.bottleneck.add(
                ...
                Conv1x1(oup, linear=True, prefix='linear_')
            )
    def hybrid_forward(self, F, x):
...
def ExpandedConvSequence(t, inp, oup, repeats, first_strides, **kwargs):
    ...
    with seq.name_scope():
        ...
        for _ in range(1, repeats):
            seq.add(ExpandedConv(cur_inp, oup, t, 1))
    return seq


class MobileNetV2(nn.HybridSequential):
    def __init__(self, num_classes=1000, width_mult=1.0, **kwargs):
        ...
        self.interverted_residual_setting = [
            # t,  c, n ,s
            [ 1, 16, 1, 1, "stage1_0_" ],   # -> 112x112
            [ 6, 24, 2, 2, "stage1_1_" ],   # -> 56x56
            [ 6, 32, 3, 2, "stage2_" ],     # -> 28x28
            [ 6, 64, 4, 2, "stage3_" ],     # -> 14x14
            [ 6, 96, 3, 1, "stage4_0_" ],   # -> 14x14
            [ 6, 160,3, 2, "stage4_1_" ],   # -> 7x7
            [ 6, 320,1, 1, "stage5_0_" ],   # -> 7x7
        ]
        ...
        with self.name_scope():
            ...
            for t, c, n, s, prefix in self.interverted_residual_setting:
                ...
                inp = oup
            self.features.add(Conv1x1(self.last_channels, prefix='stage5_1_'))
